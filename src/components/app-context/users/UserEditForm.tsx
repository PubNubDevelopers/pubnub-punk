...
import { UserMetadata, CustomField } from '@/types/app-context';
import { validateCustomFields, parseCustomFieldsFromObject } from '@/utils/app-context';

interface UserEditFormProps {
  user: UserMetadata | null;
  onSave: (userData: Partial<UserMetadata>) => Promise<void>;
  onCancel: () => void;
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Object.keys(updateData).forEach(key => {
        if (updateData[key as keyof UserMetadata] === '' && key !== 'id') {
          delete updateData[key as keyof UserMetadata];
        }
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <Input
            ...
            placeholder={user ? "" : "Enter unique user ID"}
          />
          {user && (
        ...
        <div className="col-span-1">
          ...
          <Input
            ...
            placeholder="Enter user type (e.g., admin, member)"
          />
        </div>
      ...
      <div className="flex justify-end space-x-2 pt-4">
        ...
        <Button 
          type="submit" 
          disabled={loading || hasValidationErrors || hasDuplicateKeys || (!user && !formData.id.trim())}
